<template>
  <div class="qingxieDataPage">
    <el-tabs v-model="activeName">
      <el-tab-pane label="无线伸缩节传感器数据图" name="1" v-if="dataInfo">
        <div class="data-top">
          <img class="data-image" src="../../assets/images/shen-suo.png" />
          <div>
        ...
        <div class="data-detail">
          <div class="data-chart">
            ...
            <div class="chart-num">伸缩量 {{dataInfo.datavalue}}mm</div>
            <div class="chart-img">
              <div class="line1" :style="{left: dataInfo.datavalue*3 + 'px'}"></div>
              <div class="line2"></div>
          ...
          <div class="data-table">
            <div class="table-item">
              <div class="item-name">伸缩量/mm</div>
...
export default {
  ...
  data() {
	...
	computed: {
		temperatureData() {
			...
			let unit = '';
			return `伸缩量：最大值：${maxN}${unit} 最小值：${minN}${unit} 平均值：${mean.toFixed(3)}${unit}`;
		},
	},
  mounted() {
  ...
  methods: {
		async getData() {
			 this.dataInfo = await api.telescopicJointDataInfo({
				deviceId: this.deviceNumber
			});
			
		},
		async getList() {
			this.tableData = await api.telescopicJointHistoryList({
				deviceId: this.deviceNumber,
      ...
      this.cutList();
		...
		async tableExport() {
		  this.onload = true;
		  await exportExcel({
		    url: 'ygwltelescopicjointhistory/exportExcel',
		    params: {
		      pageNum: 1,
		      pageSize: 999999999,
