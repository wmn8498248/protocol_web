  ...
  <div class="dataAnalysisPage">
    ...
    <div class="search-container">
      <el-form label-width="200px" :model="info" :rules="rulesAnalysis">
        ...
        <el-form-item label="与网关通信Id" required prop="sensorId">
          <el-input
            type="text"
            v-model="info.sensorId"
            :readonly="type == 'edit'"
          ></el-input>
        </el-form-item>
        ...
        <el-form-item label="离线时间阈值(h)" prop="onlineThreshold">
...
export default {
  ...
  methods: {
    ...
    async toSave() {
      ...
      validator.add(this.info.sensorId, ["isNonEmpty", "与网关通信Id不能为空"]);
      ...
      validator.add(this.info.onlineThreshold, ["isNonEmpty", "离线时间阈值不能为空"]);
      ...
      if (msg) {
        ...
        if (this.type == "edit") {
          await api.temperatureUpdate({
            ...this.info,
          });
        } else {
          await api.temperatureSave({
            ...this.info,
          });
        }
