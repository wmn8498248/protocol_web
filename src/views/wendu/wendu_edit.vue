<template>
  <div class="dataAnalysisPage">
    <div class="title">
      <div>{{type == 'edit' ? '温度电流传感器修改' : '温度电流传感器添加'}}</div>
    </div>
    <div class="search-container">
      <el-form label-width="160px" :model="info">
        <el-form-item label="传感器类型"> 温度电流传感器 </el-form-item>
        <el-form-item label="项目类型" required>
          <el-select v-model="info.projectModel">
            <el-option label="温度和电流设备" value="温度和电流设备"></el-option>
            <el-option label="温度设备" value="温度设备"></el-option>
            <el-option label="电流设备" value="电流设备"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="传输协议" required="">
          <el-select v-model="info.transportProtocol">
            <el-option label="NB" value="NB"></el-option>
            <el-option label="MQTT" value="MQTT"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="项目及编号" required="">
          <el-input type="text" v-model="info.deviceNumber"></el-input>
        </el-form-item>
        <el-form-item label="汇集单元识别码" required="">
          <el-input type="text" v-model="info.nodeId"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async toSave() {
      ...
      validator.add(this.info.projectModel, ["isNonEmpty", "项目类型不能为空"]);
      validator.add(this.info.transportProtocol, ["isNonEmpty", "传输协议不能为空"]);
      validator.add(this.info.deviceNumber, ["isNonEmpty", "项目及编号不能为空"]);
      validator.add(this.info.nodeId, ["isNonEmpty", "汇集单元识别码不能为空"]);
      ...
      if (msg) {
        this.$message.warning(msg);
				...
				if (this.type == 'edit') {
				  await api.temperatureElectricUpdate({
						...this.info,
						deviceId: this.deviceId
					});
				} else {
				  await api.temperatureElectricSave({
						...this.info,
