<template>
  <div class="dataAnalysisPage">
    ...
    <el-form label-width="140px" class="data_form">
      ...
      <el-form-item
        label="指定传感器"
        v-if="choosenType == 2 && projectId != '' && sensorType != ''"
      >
    ...
    <el-table
      ...
      stripe
      style="width: 100%"
      v-else-if="sensorType == '6'"
    >
      <el-table-column prop="updateTime" label="时间"></el-table-column>
      <el-table-column prop="temperature" label="温度/℃"> </el-table-column>
      <el-table-column prop="electricCurrent" label="电流/mA">
      </el-table-column>
    </el-table>
    <el-table
      :data="tableList"
    ...
    <el-table
      ...
      stripe
      style="width: 100%"
      v-else-if="sensorType == '8'"
    >
      <el-table-column prop="updateTime" label="时间"></el-table-column>
      <el-table-column prop="temperature" label="温度（°C）"> </el-table-column>
      <el-table-column prop="pressure" label="压力值(MPa)"> </el-table-column>
    </el-table>
    <el-table
      :data="tableList"
      stripe
      style="width: 100%"
      v-else-if="sensorType == '9'"
    >
      <el-table-column prop="updateTime" label="时间"></el-table-column>
    ...
    <el-table
      ...
      stripe
      style="width: 100%"
      v-else-if="sensorType == '12'"
    >
      <el-table-column prop="updateTime" label="时间"></el-table-column>
      <el-table-column label="动作次数">
        <template slot-scope="{ row }">
          A: {{ row.actionA || "--" }}, B: {{ row.actionB || "--" }}, C:
          {{ row.actionC || "--" }},
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 获取数据
    async getList() {
      let obj = this.deviceIds.find((ele) => {
        return ele.deviceId == +this.sensorName;
      });
      let query = {};
      let api = "";
      if (this.sensorType == "1") {
    ...
    initEchart() {
      ...
      if (this.sensorType == "1") {
        ...
        let fftId = this.tableData
          .map((item) => {
            return item.fftId || "--";
          })
          .reverse();
        let actionA = this.tableData
          .map((item) => {
            return item.actionA || "0";
          })
          .reverse();
        let actionB = this.tableData
          .map((item) => {
            return item.actionB || "0";
          })
          .reverse();
        let actionC = this.tableData
          .map((item) => {
            return item.actionC || "0";
          })
          .reverse();
        let changeArr = (arr) => {
          let list = [];
