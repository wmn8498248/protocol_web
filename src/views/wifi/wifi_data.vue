<template>
  <div class="qingxieDataPage">
    <el-tabs v-model="activeName">
      <el-tab-pane label="电流数据图" name="1" v-if="dataInfo">
        <div class="data-top">
        ...
        <div class="data-detail">
          <div class="data-chart">
            <div class="data-name">
              <svg-icon icon-class="dianliang" style="font-size: 20px"></svg-icon>
              电量
              <span class="data-num"> {{ dataInfo.elect }}% </span>
            </div>
            <img class="chart-img" src="../../assets/images/001.gif" />
            <div class="chart-num">{{ dataInfo.current }}A</div>
            <div>{{ dataInfo.collectionTime }}</div>
          </div>
          <div class="data-table">
            <div class="table-item">
              <div class="item-name">电流值（A）</div>
              <div>{{ dataInfo.current }}</div>
            </div>
            <div class="table-item">
              <div class="item-name">电量（%）</div>
              <div>{{ dataInfo.elect }}</div>
            </div>
      ...
      <el-tab-pane label="历史数据" name="2">
        <div class="history-content">
          ...
          <el-table :data="tableList" stripe style="width: 100%">
            ...
            <el-table-column prop="current" label="电流值/A">
            </el-table-column>
...
export default {
  ...
  computed: {
    temperatureData() {
      ...
      let unit = "°C";
      return `电流值：最大值：${maxN}${unit} 最小值：${minN}${unit} 平均值：${mean.toFixed(
        3
      )}${unit}`;
    },
  },
