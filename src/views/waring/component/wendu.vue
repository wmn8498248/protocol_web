<template>
	<div class="alarm_component">
		...
		<el-table :data="tableData" stripe style="width: 100%">
		  <el-table-column prop="projectNumber" label="项目及编号"> </el-table-column>
		  <el-table-column prop="deviceCode" label="汇集单元识别码"> </el-table-column>
		  ...
		  <el-table-column label="报警类型">
				<template slot-scope="{ row }">
				  <span style="color: #f2fa14;">--</span>
				</template>
			</el-table-column>
		  <el-table-column prop="alarmyTime" label="站点">
				<template slot-scope="{ row }">
		  ...
		  <el-table-column label="操作" width="250">
				<template slot-scope="{ row }">
				  <el-button
				    class="btn-detail"
				    size="mini"
				    @click="toDetail(row.deviceId, row.deviceCode)"
				    >查看详情</el-button
				  >
				  <el-button
				    class="btn-clear"
...
export default {
  ...
  methods: {
    async getList() {
      let { alarmList, count } = await api.temperatureElectricAlarmList({
        ...this.searchModel,
        ...this.pages,
      });
    ...
    clearAll() {
			this.$confirm('确定清除所有报警信息?', '提示', {
				...
				await api.temperatureElectricAlarmEliminate({
					status : 1
				});
