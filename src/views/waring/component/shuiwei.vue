		  ...
		  <el-form inline :model="searchModel" label-width="100px">
		    ...
		    <el-form-item>
		      ...
		      <el-button @click="clearAll" class="btn-clear" size="mini" style="height: 36px">清除所有报警信息</el-button>
					<el-button @click="tableExport" class="btn-search">导出表格</el-button>
		    </el-form-item>
		  </el-form>
		</div>
		<el-table :data="tableData" stripe style="width: 100%">
		  <el-table-column prop="deviceNumber" label="传感器编号"> </el-table-column>
		  <el-table-column prop="deviceName" label="传感器别名"> </el-table-column>
		  <el-table-column label="水位值m">
...
export default {
  ...
  methods: {
    async getList() {
      let { alarmList, count } = await api.waterLevelAlarmList({
        ...this.searchModel,
        ...this.pages,
      });
    ...
    clearAll() {
			this.$confirm('确定清除所有报警信息?', '提示', {
				...
				await api.waterLevelAlarmEliminate({
					status : 1
				});
