<template>
  <div class="dataAnalysisPage">
    <div class="title">
      <div>
        {{ type == "edit" ? "无线变形传感器修改" : "无线变形传感器添加" }}
      </div>
    </div>
    <div class="search-container">
      <el-form label-width="160px" :model="info" :rules="rulesAnalysis">
        <el-form-item label="传感器类型"> 无线变形传感器 </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async getInfo() {
      ...
      this.info = await api.deformationInfo(data);
    ...
    async toSave() {
      ...
      if (msg) {
        ...
        if (this.type == "edit") {
          await api.deformationUpdate({
            ...this.info,
          });
        } else {
          await api.deformationSave({
            ...this.info,
          });
        }
