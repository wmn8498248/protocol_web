<template>
  <div class="bileiPage">
    ...
    <el-table :data="tableData" stripe>
      ...
      <el-table-column prop="onoff1" label="单火开关1">
        <template slot-scope="{ row }">
            <el-tag type="info" size="mini" v-if="row.onoff1 === 0"> 关闭 </el-tag>
            <el-tag type="success" size="mini" v-else> 开启 </el-tag>
        </template>
      </el-table-column>
      
      <el-table-column prop="onoff2" label="单火开关2">
        <template slot-scope="{ row }">
            <el-tag type="info" size="mini" v-if="row.onoff2 === 0"> 关闭 </el-tag>
            <el-tag type="success" size="mini" v-else> 开启 </el-tag>
        </template>
      </el-table-column>

      <el-table-column prop="onoff3" label="单火开关3">
        <template slot-scope="{ row }">
            <el-tag type="info" size="mini" v-if="row.onoff3 === 0"> 关闭 </el-tag>
            <el-tag type="success" size="mini" v-else> 开启 </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="electricity" label="电流">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      let { records, total } = await api.onoffList(data);
      this.tableData = records;
      this.total = total;
    },
    async toSet(deviceId) {
      this.$router.push({
        path: "/manage/onoff_setting",
        query: {
          deviceId,
        },
      });
