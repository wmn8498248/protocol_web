            ...
            <div class="warp" style="height: initial">
              <ul class="item">
                <li>
                  ...
                  <span class="title" v-text="'长度(mm)'"></span>
                  <span class="title" v-text="'电压(V)'"></span>
...
export default {
  ...
  methods: {
    ...
    async getList(status, res) {
      ...
      switch (status) {
        case "日":
          historyList = await api.tcDeviceHistoryList({
      ...
      let hisTitle = this.deviceNumberName;
      this.chart4Refresh(this.historyList, hisTitle);
    },
    // 导出表格
    async tableExport() {
      this.onload = true;
    ...
    async handleGoToDetail(res) {
      ...
      this.deviceNumber = this.dataDetailsList[res.target.title].deviceNumber;
      this.deviceNumberName = this.dataDetailsList[res.target.title].deviceName;

      this.getList(
        this.times,
        this.dataDetailsList[res.target.title].deviceName
      );
    },
    parentRouting() {
    ...
    chart4Refresh(res, hisTitle) {
      ...
      let dateTime = res;
      let timeList = [];
      let dateListTemperature = []; // 湿度
      let dateListPressure = []; //
      let datavalueList = [];

      dateTime.forEach((element) => {
        dateListTemperature.push(parseFloat(element.temdev));
        dateListPressure.push(parseFloat(element.voltage));
        datavalueList.push(parseFloat(element.datavalue));
        timeList.push(element.updateTime); // 时间
      });
    ...
    async getData() {
      const { deviceNum, deviceType, voltLevel, list } = await api.homeSf6({
        projectId: 2,
      });
      ...
      if (list.length == 0) {
        ...
        this.deviceNumberName = list[0].deviceName;
        this.getList(that.times, list[0].deviceName);
      }
